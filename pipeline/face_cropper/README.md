...
> `pipeline/face_cropper/` re-exports `FaceDetector` from there — it does not reimplement detection. Behaviour changes (confidence threshold, "largest face" heuristic, etc.) belong in the model service file, not here.
...
# A whole dataset
python -m pipeline.face_cropper crop-dir ./raw_dataset ./crops \
    --recursive \
    --resize 224 \
    --padding 0.1 \
    --skip-existing \
...
```bash
python pipeline/face_cropper/test_face_cropper.py /path/to/a/face.jpg
```
